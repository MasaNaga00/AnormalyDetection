...
# 既知ラベルでの検出遅れ（任意）
# ============================================================================
def _months_between(ym_from: int, ym_to: int) -> int:
    """YYYYMM 同士の月差（ym_to - ym_from、月数）。"""
    y1, m1 = divmod(int(ym_from), 100)
    y2, m2 = divmod(int(ym_to), 100)
...
def measure_labeled(basis: list[dict], labels: pd.DataFrame, cfg: dict,
                    ...
                    alpha: float = 0.005, min_count: int = 3, burst_window: int = 0,
                    lookback_m: int = 6, verbose: bool = True) -> pd.DataFrame:
    """labels: 列 [事業コード, 開発コード, 部番, 発生年月]。

    各既知異常について「発生年月の lookback_m ヶ月前」から最初に発火した月を探し、
    発生年月に対する相対月数を『遅れ月』として返す。
      遅れ月 < 0 … 販社報告（発生年月）より【早く】検知できた（早期検知）
      遅れ月 = 0 … 発生年月ちょうどに検知
      遅れ月 > 0 … 発生年月より後追いで検知
    ラベルの発生年月は多くの場合「販社が気づいた月」であり、統計検知が
    それより先行するのは狙い通りなので、負値は成功として読む。

    detector='drift' は CUSUM、'spike' は spike_test。未検出は遅れ NaN。
    出力にはパラメータ列（detector, R, h, alpha, min_count, burst_window,
    lookback_m）も残す（どの設定での結果か CSV 単体で分かるように）。"""
    bmap = {b["key"]: b for b in basis}
    param_cols = dict(detector=detector, R=R, h=h, alpha=alpha,
    ...
    for _, r in labels.iterrows():
        ...
        if b is None or onset_ym is None:
            rows.append(dict(事業コード=key[0], 開発コード=key[1], 部番=key[2],
                             発生年月=onset_ym, 検出=False, 遅れ月=np.nan,
                             備考="監視対象外/保留", **param_cols))
            continue
        # 発生年月の位置。ちょうどの月が無ければ直後の月を基準にする
        # （その場合も遅れ月は onset_ym 起点の月数で数える）。
        ym = b["ym"]
        pos_exact = np.flatnonzero(ym == onset_ym)
        note = ""
        if pos_exact.size:
            onset = int(pos_exact[0])
            offset = 0  # ym[onset] と onset_ym の差（月数）
        else:
            pos_after = np.flatnonzero(ym >= onset_ym)
            if pos_after.size == 0:
                rows.append(dict(事業コード=key[0], 開発コード=key[1], 部番=key[2],
                                 発生年月=onset_ym, 検出=False, 遅れ月=np.nan,
                                 備考="発生月が監視レンジ外", **param_cols))
                continue
            onset = int(pos_after[0])
            offset = _months_between(onset_ym, int(ym[onset]))
            note = f"発生月が系列に無く{int(ym[onset])}を基準"
        if detector == "drift":
        ...
        search_from = max(0, onset - int(lookback_m))
        idx = np.flatnonzero(np.nan_to_num(np.asarray(alarm, float)[search_from:]))
        detected = idx.size > 0
        delay = (search_from + int(idx[0]) - onset + offset) if detected else np.nan
        rows.append(dict(事業コード=key[0], 開発コード=key[1], 部番=key[2],
                         発生年月=onset_ym, 検出=bool(detected),
                         遅れ月=delay, 備考=note, **param_cols))
    out = pd.DataFrame(rows)
    if verbose and not out.empty:
        det = out["検出"].mean()
        med = out.loc[out["検出"], "遅れ月"].median() if out["検出"].any() else np.nan
        early = int((out["遅れ月"] < 0).sum())
        print(f"[ラベル評価 {detector} R={R} h={h}] 検出率={det:.2f} "
              f"遅れ中央値={med} 早期検知(負)={early}件")
    return out


def measure_labeled_grid(basis: list[dict], labels: pd.DataFrame, cfg: dict,
                         bt: dict = BT, detector: str = "drift",
                         lookback_m: int = 6) -> pd.DataFrame:
    """グリッド全点でラベル評価を回し、1枚の表に縦積みして返す。
    detector='drift': R_grid × h_grid。detector='spike': alpha×min_count×burst。
    どの行がどのパラメータかは列で分かる。"""
    frames = []
    if detector == "drift":
        for R in bt["R_grid"]:
            for h in bt["h_grid"]:
                frames.append(measure_labeled(basis, labels, cfg, R=R, h=h,
                                              detector="drift",
                                              lookback_m=lookback_m, verbose=False))
    else:
        for alpha in bt["alpha_grid"]:
            for mc in bt["mincount_grid"]:
                for bw in bt["burst_grid"]:
                    frames.append(measure_labeled(
                        basis, labels, cfg, R=np.nan, h=np.nan, detector="spike",
                        alpha=alpha, min_count=mc, burst_window=bw,
                        lookback_m=lookback_m, verbose=False))
    out = pd.concat(frames, ignore_index=True)
    # パラメータ点ごとのサマリも出す
    keys = ["R", "h"] if detector == "drift" else ["alpha", "min_count", "burst_window"]
    summ = (out.groupby(keys, dropna=False)
               .agg(検出率=("検出", "mean"),
                    遅れ中央値=("遅れ月", "median"),
                    早期検知件数=("遅れ月", lambda x: int((x < 0).sum())))
               .reset_index())
    with pd.option_context("display.width", 200):
        print(f"=== ラベル評価サマリ（{detector}, グリッド全点） ===")
...
def run_backtest(panel_path: str, cfg: dict, labels_path: str | None = None,
    ...
    if labels_path:
        ...
        # ドリフトのグリッド全点でラベル評価（どの行がどの (R,h) かは列で分かる）。
        # 遅れ月は発生年月（＝販社報告月）基準で、負値＝報告より早く検知（早期検知）。
        lab = measure_labeled_grid(basis, labels, cfg, bt=bt,
                                   detector="drift", lookback_m=6)
